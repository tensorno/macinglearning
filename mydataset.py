...
class BikeRentalDataset(Dataset):
    def __init__(self, csv_file, seq_length=24, json_file=None):
    ...
    def __getitem__(self, idx):
        ...
        target = torch.tensor(self.targets.iloc[idx + self.seq_length - 1].values.astype(float), dtype=torch.float32)
...
class BikeDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return torch.tensor(self.features.iloc[idx].values.astype(float), dtype=torch.float32), torch.tensor(self.targets.iloc[idx].values.astype(float), dtype=torch.float32)
